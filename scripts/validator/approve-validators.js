...
async function main() {
    ...
    // Process all records that are not yet confirmed approved
    ...
    for (const record of candidates) {
        ...
        // Always check getProposal — never rely on local approveTxHash
        ...
        if (state === PROPOSAL.APPROVED) {
            // Already approved on-chain — sync local status
            record.status = 'approved';
            records[idx]  = record;
            saveOutput(records);
            console.log(`    Already approved on-chain — status updated`);
            approved++;
            continue;
        }

        if (state === PROPOSAL.NOT_FOUND) {
            console.log(`    No proposal found — not registered or already withdrawn, skipping`);
            skipped++;
            continue;
        }
        ...
        // PENDING_APPROVAL — call approve
        try {
            const op = await sdk.operation.contractInvokeByGasOperation({
                contractAddress: DPOS_CONTRACT, sourceAddress: COMMITTEE_ADDRESS, gasAmount: '0',
                input: JSON.stringify({ method: 'approve', params: { operate: 'apply', item: 'validator', address: pool.address } }),
            });
            if (op.errorCode !== 0) throw new Error(`contractInvokeByGasOperation failed [${op.errorCode}]`);

            const hash = await submitTx(COMMITTEE_ADDRESS, COMMITTEE_KEY, op.result.operation);
            console.log(`    Approve tx: ${hash}`);
            await waitForTx(hash);

            // Verify on-chain after tx
            const stateAfter = await getProposalState(sdk, DPOS_CONTRACT, pool.address);
            record.approveTxHash = hash;
            record.status        = stateAfter === PROPOSAL.APPROVED ? 'approved' : 'applied';
            records[idx]         = record;
            saveOutput(records);
            console.log(`    Verified state after approve: ${stateAfter}`);
            console.log(`  Done ✓`);
            approved++;
        } catch (err) {
    ...
    console.log(`\nCompleted. Approved: ${approved} | Skipped: ${skipped} | Failed: ${failed}`);
}
